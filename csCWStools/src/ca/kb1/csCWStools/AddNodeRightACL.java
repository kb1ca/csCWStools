...
public class AddNodeRightACL {
	...
	// somewhere to efficiently track the users who we're targeting to be updated or added
	static class RightID
	{
		// what is the Content Server ID for the user who's ACL we're targeting?
		private long rightID;
		
		// did we manage to update an existing ACL for this user, or do we need to add a new one?
	...
	public static String addNodeRightACL(Node thisNode, DocumentManagement docManClient) {
		...
		// otherwise attach the NodePermissions bits to a new user ACL
	    NodeRight newNodeACLRight = new NodeRight();
	    newNodeACLRight.setType("ACL");
	    newNodeACLRight.setPermissions(newNodePermissions);

	    // and then add a user ACL for each of the users that didn't already have an ACL to update
		for (RightID thisTargetRightID : targetRightIDs) {
